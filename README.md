...
Generate seamlessly looping scrolling animations for small OLED displays from any image.

Built for the OLED panel on the **SteelSeries Apex Pro** keyboard (128x40), but works with any display size or use case that needs a scrolling black-and-white GIF.
...
```sh
uv run oled_scroll.py logo.png
```

This produces `oled_scroll.gif` — a 128x40 black-and-white animation at 10 FPS with seamless looping.
...
| `-o`, `--output` | `oled_scroll.gif` | Output GIF path |
...
```sh
uv run oled_scroll.py my-logo.png
```

**Custom display size:**

```sh
uv run oled_scroll.py my-logo.png --width 256 --height 64
```

**Image already white-on-black:**

```sh
uv run oled_scroll.py white-on-black-logo.png --no-invert
```

**Faster scroll, custom output path:**

```sh
uv run oled_scroll.py my-logo.png --speed 6 -o fast-scroll.gif
```
