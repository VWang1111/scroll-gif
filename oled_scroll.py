...
# dependencies = ["Pillow"]
# ///
"""Generate a scrolling OLED animation GIF from any image.

Converts an input image to a black-and-white scrolling animation sized for
small OLED displays. The image scrolls continuously right-to-left with
seamless wrapping.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a scrolling OLED animation GIF from any image.",
    )
    parser.add_argument(
        "image",
        type=Path,
    ...
    parser.add_argument(
        "-o", "--output",
        type=Path,
        default=Path("oled_scroll.gif"),
        help="output GIF path (default: oled_scroll.gif)",
    )
    parser.add_argument(
        "--width",
        type=int,
